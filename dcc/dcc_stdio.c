...
static void dbg_write(unsigned long dcc_data)
{
    int len = 4;
...
void dbg_write_u32array(const unsigned long *val, long len)
{
...
void dbg_write_u32(unsigned long val)
{
    dbg_write(TARGET_REQ_DEBUGMSG_HEXMSG(4) | (1 << 16));
    dbg_write(val);
}

void dbg_write_u16array(const unsigned short *val, long len)
{
...
void dbg_write_u16(unsigned short val)
{
    dbg_write(TARGET_REQ_DEBUGMSG_HEXMSG(2) | (1 << 16));
    dbg_write(val);
}

void dbg_write_u8array(const unsigned char *val, long len)
{
    unsigned long dcc_data;
...
void dbg_write_u8(unsigned char val)
{
    dbg_write(TARGET_REQ_DEBUGMSG_HEXMSG(1) | (1 << 16));
    dbg_write(val);
}
